	...
	public NbtPlacerUtil spawnEntity(Tag nbtElement, WorldGenRegion region, BlockPos offset, BlockPos from, BlockPos to,
		...
		if (optionalEntity.isPresent()) {
			...
			if (entity instanceof HangingEntity deco) {
				double newX = realPosition.x() - (deco.getBbWidth() % 32 == 0 ? 0.5 : 0.0) * deco
					.getDirection()
					.getCounterClockWise()
					.getStepX();
				double newY = realPosition.y() - (deco.getBbHeight() % 32 == 0 ? 0.5 : 0.0);
				double newZ = realPosition.z() - (deco.getBbWidth() % 32 == 0 ? 0.5 : 0.0) * deco
					.getDirection()
					.getCounterClockWise()
					.getStepZ();
